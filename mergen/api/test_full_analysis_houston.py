#!/usr/bin/env python3
"""Test full analysis for Houston - dokümanlardan çıkan lokasyona göre."""
import requests
import time

API_BASE = "http://localhost:8000"
OPP_ID = 38  # FIFA World Cup (title'da Houston geçiyor)

print("=" * 70)
print("🚀 TAM ANALİZ TEST - HOUSTON (Dokümanlardan Otomatik)")
print("=" * 70)
print()
    ...
    for i in range(30):
        ...
        if status_resp.status_code == 200:
            ...
            if status == 'completed':
                ...
                # Step 2: Start Hotel Match (NO options - let it auto-detect from title)
                print(f"\n🏨 2. Hotel Match başlatılıyor (otomatik şehir tespiti)...")
                hotel_resp = requests.post(
                    f"{API_BASE}/api/pipeline/run",
                    json={
                        "opportunity_id": OPP_ID,
                        "analysis_type": "hotel_match"
                        # NO options - sistem title'dan Houston'ı bulmalı
                    }
                )
                        ...
                        if hotel_status_resp.status_code == 200:
                            ...
                            if hotel_status == 'completed':
                                ...
                                if requirements.get('city_code') == 'HOU':
                                    ...
                                    print(f"   ⚠️ YANLIŞ ŞEHİR: {requirements.get('city_code')} (Houston olmalıydı)")
