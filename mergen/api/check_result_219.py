#!/usr/bin/env python3
"""Check Hotel Match result ID 219."""
import sys
...
db = SessionLocal()
try:
    res = db.query(AIAnalysisResult).filter(AIAnalysisResult.id == 219).first()
    
    if not res:
        print("❌ ID 219 bulunamadı!")
        exit(1)
    
    print(f'\n{"="*60}')
    print(f'🏨 HOTEL MATCH SONUCU (ID: 219)')
    print(f'{"="*60}')
